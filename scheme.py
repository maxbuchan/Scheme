...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('Incorrect number of vals')
        while formals:
            value, symbol = vals.first, formals.first
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        formal_parameters = target.second
        check_formals(formal_parameters)
        env.define(target.first, LambdaProcedure(formal_parameters, expressions.second, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if not scheme_symbolp(binding.first):
            raise SchemeError
        parameters = Pair(binding.first, parameters)
        vals = Pair(scheme_eval(binding.second.first, env), vals)
        bindings = bindings.second
    return env.make_child_frame(parameters, vals)
    # END PROBLEM 15
